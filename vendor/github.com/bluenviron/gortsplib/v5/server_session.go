...
func (ss *ServerSession) handleRequestInner(sc *ServerConn, req *base.Request) (*base.Response, error) {
	...
	case base.Setup:
		...
		if res.StatusCode == base.StatusOK {
			...
			if existing, ok := ss.setuppedMedias[medi]; ok {
				// Some RTSP clients, including go2rtc when adding an ONVIF
				// backchannel during a reconnect, repeat SETUP for a media that
				// is already configured on the same session. Treat an identical
				// TCP SETUP as idempotent and return the existing transport.
				if ss.state == ServerSessionStatePrePlay && medi.IsBackChannel &&
					inTH.Profile == headers.TransportProfileAVP &&
					protocol == ProtocolTCP && inTH.InterleavedIDs != nil &&
					existing.tcpChannel == inTH.InterleavedIDs[0] {
					th := headers.Transport{
						...
						InterleavedIDs: &[2]int{
							existing.tcpChannel,
							existing.tcpChannel + 1,
						},
					}
					if len(medi.Formats) == 1 {
						if streamFormat, ok := stream.medias[medi].formats[medi.Formats[0].PayloadType()]; ok {
							th.SSRC = &streamFormat.localSSRC
						}
					}
					if res.Header == nil {
						res.Header = make(base.Header)
			...
			if protocol == ProtocolTCP && inTH.InterleavedIDs != nil &&
				ss.isChannelPairInUse(inTH.InterleavedIDs[0]) {
				return &base.Response{
					StatusCode: base.StatusBadRequest,
